...
} from '../../provider/services/library-invoice-line.util';
import { AdminApiService } from '../services/admin-api.service';
import type { ProviderAccountResponse } from '../services/admin.types';

@Component({
  selector: 'app-admin-library-invoice-create-page',
  standalone: true,
  imports: [ReactiveFormsModule, RouterLink, DecimalPipe],
  templateUrl: './admin-library-invoice-create-page.component.html',
})
export class AdminLibraryInvoiceCreatePageComponent implements OnInit {
  ...
  readonly form = this.fb.nonNullable.group({
    ownerUserId: [null as unknown as number, Validators.required],
  ...
  submit(): void {
    ...
    this.inventory
      ...
      .subscribe({
        next: (created) => {
          void this.router.navigate(['/admin/facturas-libreria', created.id]);
        },
        error: () => {
          this.saving.set(false);
          this.error.set('No se pudo crear la factura (revisa número único y datos).');
        },
      });
  }
}
